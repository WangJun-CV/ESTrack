...
class BaseTrainer:
    ...
    def __init__(self, actor, loaders, optimizer, settings, lr_scheduler=None):
        ...
        self.epoch = 0   #测试终止时，可以改为终止的权重轮数
        self.stats = {}
                ...
                for epoch in range(self.epoch+1, max_epochs+1):
                    ...
                    if   epoch >= int(max_epochs*0.8)-1:
                        if self._checkpoint_dir:
                            if self.settings.local_rank in [-1, 0]:
                                self.save_checkpoint(eval=False)   #eval=True
                    elif epoch > (max_epochs - 1) or save_every_epoch or epoch % 1 == 0 or epoch in save_epochs:
                  # if epoch > (max_epochs - 1) or save_every_epoch or epoch % 3 == 0 or epoch in save_epochs or epoch > ( max_epochs - 5):
    ...
    def save_checkpoint(self, eval=False):
    ...
    def delet_old_checkpoint(self, epoch_before=0):
        ...
        checkpoint_list_to_remove = [ckpt_name for ckpt_name in  checkpoint_list if int(ckpt_name[-12:-8]) < epoch_before]
