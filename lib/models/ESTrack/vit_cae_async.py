...
class LocalIntegration(nn.Module):
    def __init__(self, dim, ratio=1, act_layer=nn.ReLU, norm_layer=nn.GELU):
        ...
        self.conv1 = nn.Conv2d(dim, mid_dim, 1, 1, 0)
        self.norm = norm_layer(mid_dim)
        self.conv2 = nn.Conv2d(mid_dim, mid_dim, 3, 1, 1, groups=mid_dim)
        self.act = act_layer()
        self.conv3 = nn.Conv2d(mid_dim, dim, 1, 1, 0)
...
class AdditiveTokenMixer(nn.Module):
    ...
    def forward(self, x):
        ...
        #print(x_.shape,1)  #torch.Size([32, 768, 8, 8])
        # H = W = int(N ** 0.5)
       # x_ = x.reshape(1, B, N, C).permute(0, 2, 1, 3)
        #print(x_.shape)
       ...
       # q = self.qkv(x_)
        ...
        out = self.proj_drop(out)   ##torch.Size([32, 768, 8, 8])
        out = out.reshape(B, C, N).transpose(1, 2)
        return out
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., seperate_self_attn=False,
                 ...
                 add_cls_token=False):
        super().__init__()
        self.local_perception = LocalIntegration(dim=dim, ratio=1,act_layer=nn.GELU, norm_layer=nn.LayerNorm)
        ...
        self.attn1 = AdditiveTokenMixer(dim=dim, attn_bias=False, proj_drop=drop)
        if init_values > 0:
