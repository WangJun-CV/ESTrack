...
from lib.models.ESTrack.vit_cae_async import CAE_Base_patch16_224_Async
...
class ESTrack(nn.Module):
    """ This is the base class for ESTrack """
...
def build_ESTrack(cfg, training=True):

    current_dir = os.path.dirname(os.path.abspath(__file__))  # This is your Project Root
    pretrained_path = os.path.join(current_dir, '../../../pretrained_models')
    if cfg.MODEL.PRETRAIN_FILE and ('ESTrack' not in cfg.MODEL.PRETRAIN_FILE) and training:
        pretrained = os.path.join(pretrained_path, cfg.MODEL.PRETRAIN_FILE)
    else:
        pretrained = ''

    if cfg.MODEL.BACKBONE.TYPE == 'cae_vit_base_patch16_224':
    ...
    if 'ESTrack' in cfg.MODEL.PRETRAIN_FILE and training:
