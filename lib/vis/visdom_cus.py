...
class VisHeatmap(VisBase):
    ...
    def draw_data(self):
        ...
        if len(self.raw_data) == 2:
            ...
            # self.visdom.heatmap(self.raw_data[0].clone(), opts={'title': self.title, 'xmax':1.0, 'xmin':0.0,}, win=self.title)
...
class VisHistogram(VisBase):
    ...
    def draw_data(self):
        if len(self.raw_data) == 2:
            self.visdom.bar(self.raw_data[0].clone(), win=self.title,
                            opts={
                                   **self.raw_data[1], 'ymin':0, 'ymax':1})
        else:
            bar = self.visdom.bar(self.raw_data[0].clone(), win=self.title,
                            opts=dict(title=self.title, stacked=False, ytickmin=0.0, ytickmax=1.0)
                            )
            # self.visdom.win_properties(bar, opts=dict(ytickmin=0, ytickmax=1))
        # win = self.visdom.subplots(num_rows=2, num_cols=2)
        # for i in range(4):
        #     row = i // 2
        #     col = i % 2
        #     self.visdom.histogram(self.data[i], win=win, opts=dict(title=f"Dist {i+1}"), row=row, col=col)
        # self.visdom.heatmap(self.raw_data.clone(), opts={'title': self.title}, win=self.title)
...
class VisTracking(VisBase):
    ...
    def draw_data(self):
        ...
        resize_factor = 1
        if max(disp_image.shape) > 640:
            resize_factor = 640.0 / float(max(disp_image.shape))
        ...
        if self.raw_data[1][0].shape[0] == 5:
            scores = [box[4].item() if box.shape[0]==5 else 0 for box in self.raw_data[1]]
...
class VisTrackingPts(VisBase):
    ...
    def draw_data(self):
        ...
        # if box has score
        scores = self.raw_data[1][4]
        

        box = self.raw_data[1][:4]
        pts = [resize_factor * b for b in self.raw_data[2]]
        for i, disp_rect in enumerate(pts):
            color = index_to_color(1)
            cv2.circle(disp_image, (int(disp_rect[0]),int(disp_rect[1])), 2, color, 2)
        pts = [resize_factor * b for b in self.raw_data[3]]
        for i, disp_rect in enumerate(pts):
            color = index_to_color(3)
            cv2.circle(disp_image, (int(disp_rect[0]),int(disp_rect[1])), 2, color, 2)

        color = index_to_color(0)
        cv2.rectangle(disp_image,
                        (int(box[0]), int(box[1])),
                        (int(box[0] + box[2]), int(box[1] + box[3])), color, 1)
        
        cv2.putText(disp_image, "{:.3f}".format(scores), (int(box[0]), int(box[1])),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
...
class VisPoints(VisBase):
    ...
    def draw_data(self):
        ...
        pts = [resize_factor * b for b in self.raw_data[1]]
        # pts = 

        for i, disp_rect in enumerate(pts):
            # color = ((255 * ((i % 3) > 0)), 255 * ((i + 1) % 2), (255 * (i % 5)) // 4)
            color = index_to_color(3)
            cv2.circle(disp_image, (int(disp_rect[0]),int(disp_rect[1])), 4, color, 1)
            # if scores is not None and scores[i] != 0:
            #     cv2.putText(disp_image, "{:.3f}".format(scores[i]), (int(disp_rect[0]), int(disp_rect[1])),
            #                 cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
        # for i, mask in enumerate(self.raw_data[2], 1):
...
class Visdom:
    def __init__(self, debug=0, ui_info=None, visdom_info=None, env=None):
        ...
        if env is not None:
            ...
            # self.visdom = visdom.Visdom(server='http://localhost',
            #                              port=8097, env=env)
        else:
            self.visdom = visdom.Visdom(server=visdom_info.get('server', '127.0.0.1'),
    ...
    def register(self, data, mode, debug_level=0, title='Data', **kwargs):
        if title not in self.registered_blocks.keys():
            ...
            if mode == 'image':
                ...
                self.registered_blocks[title] = VisHistogram(self.visdom, show_data, title)
                ...
                self.registered_blocks[title] = VisTrackingPts(self.visdom, show_data, title)
            else:
